...
function [control_fit_theta,control_fit_Pc,control_fit_Pt] = main_fit_data_neural_net_postprocess(net_theta,net_Pc,net_Pt,X,Y,m,g,zstar,cluster_data)
...
TE0 = 0.5*m*zstar(1)*zstar(1) + m*g*zstar(2);
    ...
    if (cluster_data == 1 && data_type ~= 1)
        for i=1:mmm
            for j=1:nnn
                vx_temp = X(i,j);
                h_temp = Y(i,j);
                TE_temp =  0.5*m*vx_temp*vx_temp + m*g*h_temp;
                %TE0
                ...
                if (data_type == 2) %Pc
                    if (TE_temp>TE0) %only save TE > TE0 for fitting  
                         ...
                         netrainX(n1,2) = Y(i,j);
                         netrainX(n1,3) = zstar(1); %velocity
                         netrainX(n1,4) = zstar(2); %apex height
...
%      end
    ...
    if (exist('index'))
        if (cluster_data == 1 && data_type ~= 1)
            for px = 1:size(index)
                iii = index(px,1); jjj = index(px,2);
                %iii = all_data(pppp).index(px,1); jjj = all_data(pppp).index(px,2);
                if (forecast(1).Y(px,1)<0)
                    forecastedData1(iii,jjj) = 0;
                else
                    forecastedData1(iii,jjj) = forecast(1).Y(px,1);
                end
            ...
            for px=1:mmm
                for qx=1:nnn
                        ...
                        forecastedData1(px,qx) = forecast(1).Y(nx,1);
        ...
        forecastedData1 = forecast(1).Y; %only 1 data point
    end
...
%     end
    clear net1
    clear netrainX forecast forecastedData1 index 
